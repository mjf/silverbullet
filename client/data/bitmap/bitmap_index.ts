...
export class BitmapIndex {
  ...
  // Structured dirty tracking: avoids string split+parseInt on flush
  ...
  // Unified bit operations (merged setBit+setBitAndCheck, clearBit+clearBitAndCheck)

  /**
   * Set a bit in a bitmap index. Returns true if the bitmap was empty before.
   */
  setBit(
    tagId: number,
    column: string,
  ...
  /**
   * Clear a bit from a bitmap index. Returns true if the bitmap is now empty.
   */
  clearBit(
    tagId: number,
    column: string,
