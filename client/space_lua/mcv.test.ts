...
test("estimateMatchFraction with MCV gives better estimate than uniform", () => {
  ...
  // MCV should estimate ~9/228 ≈ 3.9% match fraction (not 30/228 = 13.2%)
  // And ~30/9 ≈ 3.33 right rows per key
  expect(result.matchedLeftFraction).toBeLessThan(0.06);
  expect(result.matchedLeftFraction).toBeGreaterThan(0.02);
  // Output rows ≈ 228 * 0.039 * 3.33 ≈ 30 for inner, but matched = 9 pages
  // The key: matchedLeftFraction * leftRows ≈ 9
